...
表单域 要用 `fieldset` 标签包起来，并用 `legend` 标签说明表单的用途， 每个 `input` 标签对应的说明文本都需要使用 `label` 标签，并且通过为 `input` 设置 id 属性，在 `lable` 标签中设置 `<label for="input-id">` 来让说明文本和相对应的 `input` 关联起来。
